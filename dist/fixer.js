...
export const runTestFixLoop = async (scheme, destination, maxRetries = 3, refreshDestinations = false, testPlan) => {
    let retries = 0;
    ...
    while (retries < maxRetries) {
        ...
        if (!success) {
            ...
            if (failures.length === 0) {
                ...
                logger.warn('Full log tail: ' + log.slice(-500));
            }
        }
        else {
            // Check for failures
            failures = await getTestFailures(xcresultPath);
        }
        if (failures.length === 0) {
            if (success) {
                ...
                logger.error('Build failed and no structured errors could be parsed. Stopping.');
                break;
            }
        }
        logger.error(`Found ${failures.length} failure(s).`);
        // Fix the first one
...
export const fixFailure = async (failure) => {
