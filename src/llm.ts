...
let remoteConfig: LLMConfig | null = null;
...
const checkLocalConnection = async (url: string): Promise<boolean> => {
    try {
...
export const getAnalyzerClient = (): { client: OpenAI, model: string } => {
    if (!activeConfig) throw new Error('LLM not configured.');
...
export const getFixerClient = (): { client: OpenAI, model: string } => {
    if (!activeConfig) throw new Error('LLM not configured.');
