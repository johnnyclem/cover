...
/**
 * Options for PR coverage calculation.
 */
export interface PRCoverageOptions {
  /**
  ...
  /**
   * Treat non-executable lines (not in coverage data) as covered.
   * This prevents comments/blank lines from affecting coverage %.
   * Default: true
   */
  treatNonExecutableAsCovered?: boolean;
}
 ...
 */
export async function calculatePRCoverage(
  diffResult: PRDiffResult,
  coverageData: Map<string, LineCoverageData>,
  options: PRCoverageOptions = {}
): Promise<PRCoverageResult> {
  const { verbose = false, treatNonExecutableAsCovered = true } = options;

  const files: PRFileCoverage[] = [];
  let totalNew = 0;
  let totalCovered = 0;
...
function findCoverageForFile(
  ...
  let bestMatch: LineCoverageData | null = null;
  let bestScore = 0;
 ...
 */
export function shouldExcludeFromPRCoverage(filePath: string): boolean {
